...
int AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    const CChainParams& chainparams = Params();
    AssertLockHeld(cs_main);
    // Check for duplicate
    ...
    return 2; // 2 indicates it is new
}
    ...
    else if (strCommand == "reject")
    {
        if (fDebug) {
            try {
                ...
                LogPrint((strMsg == "tx") ? "tx" : (strMsg == "block") ? "block" : "net", "recv reject %s peer=%d\n", SanitizeString(ss.str()), pfrom->id);
